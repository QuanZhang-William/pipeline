...
`)}
	...
	for _, pr := range prs {
		for _, w := range pr.Spec.Workspaces {
			expectedPVCName := volumeclaim.GetPersistentVolumeClaimName(w.VolumeClaimTemplate.Name, w, *kmeta.NewControllerRef(pr))
...
func Test_taskWorkspaceByWorkspaceVolumeSource(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			binding, err := c.taskWorkspaceByWorkspaceVolumeSource(ctx, tc.pipelineWorkspaceName, tc.prName, tc.wb, tc.taskWorkspaceName, "", *kmeta.NewControllerRef(testPr))
			if err != nil {
				t.Errorf("Pipeline.taskWorkspaceByWorkspaceVolumeSource() returned unexpected error: %v", err)
			}
